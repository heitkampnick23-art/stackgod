  ...
  const sse = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        controller.enqueue(enc.encode(`data: ${JSON.stringify({ done: true, model, tokens_in: tokensIn, tokens_out: tokensOut })}\n\n`));
      } catch (e) {
...
const SYSTEM_PROMPT = `You are Stackgod, an AI app builder. Output a single self-contained HTML+Tailwind+JS file unless asked otherwise. Use modern, responsive, accessible design. Keep state in localStorage by default. Wire all buttons — never ship dead UI.`;
